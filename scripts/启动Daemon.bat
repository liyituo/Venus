...
rem ============================================================
rem  Start Daemon in background (no window, log -> .pcagent\daemon.err.log)
rem  NOTE: usually not needed - the GUI auto-starts the Daemon.
rem  Use this only for running Daemon standalone (Web console).
rem  Writes PID file (.pcagent\daemon.pid) so 停止Daemon.bat can
...
echo Daemon 已在后台启动（输出日志：%ROOT%\.pcagent\daemon.err.log，PID metadata：%PIDFILE%）。
exit /b 0
