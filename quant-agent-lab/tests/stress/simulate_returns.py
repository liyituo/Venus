...
def main() -> int:
    ...
    ap.add_argument(
        "--symbols", nargs="+", default=["AAPL", "MSFT", "GOOG", "AMZN", "META", "NVDA"]
    )
    args = ap.parse_args()
    ...
    provider = SimulatedMarketProvider(
        paths.data_dir, symbols=tuple(args.symbols), now_fn=clock.now
    )
    service.provider = provider
    strat = LlmFundamentalStrategy(
        cfg, client, RagClient(cfg.rag_url, cfg.rag_collection), audit=service.audit, clock=clock
    )
        ...
        try:
            ...
            if report.plan.risk_decision.allowed_order_ids:
                ...
                exec_result = service.execute(
                    report.report_id, mode="paper", request_id=f"sim-exec-{day}"
                )
                status = (
                    exec_result.get("execution_status", "?")
                    if isinstance(exec_result, dict)
                    else getattr(exec_result, "status", "?")
                )
