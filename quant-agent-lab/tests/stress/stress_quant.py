...
def _signal_once(client: LlmClient, i: int) -> dict:
    ...
    try:
        raw = client.complete(_SYS, _USER_TMPL.format(sym=f"T{i}"),
                              max_tokens=400, thinking_disabled=True)
        ok = _parse_llm_json(raw) is not None
        return {"ok": ok, "latency": time.perf_counter() - t0,
...
def main() -> int:
    ...
    print(f"  并发成功率: {conc_ok}/20 | 总耗时: {time.perf_counter()-t0:.2f}s "
    ...
    provider = SimulatedMarketProvider(tmp, symbols=("AAPL", "MSFT"),
                                       now_fn=fake_now)
    ...
    strat = LlmFundamentalStrategy(CFG, client, RagClient(RAG_URL, "financial-reports"),
                                   audit=service.audit, clock=service.clock)
        ...
        try:
            ...
            if report.plan.risk_decision.allowed_order_ids:
                ...
                service.execute(report.report_id, mode="paper",
                                request_id=f"stress-exec-{i}")
                ok_runs += 1
            else:
                # 无订单（全 HOLD 或风控拦截）也算链路成功（降级语义）
                ok_runs += 1
            print(f"  第{i+1}次: {status} | {time.perf_counter()-t0:.2f}s | "
