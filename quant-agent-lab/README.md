...
`quant-agent-lab` 是一个隔离、离线优先的金融决策与 Paper Trading 工程演示。它读取本地 JSON 快照，生成可审计的计划和风险检查，要求人类批准后才允许确定性 `PaperBroker` 执行，并提供一个可通过 MCP Apps 嵌入宿主 Chat 的极客终端风格交互式仪表盘。新增的 K 线与 Strategy Lab 仍只调用后端权威结果，不在浏览器生成交易事实。

这不是投资建议，不做收益承诺，不连接实时行情、券商账户、凭据、通知或真实资金。`LiveBroker` 永久禁用；任何非 `paper` 执行都会安全失败。

## 目录

```text
quant-agent-lab/
├─ config/                         # demo/risk 版本化配置
...
└─ var/                            # SQLite、离线快照、报告、审计（Git ignored）
```

父 Agent 不在 Python import path 中；所有源代码、运行时状态和 GUI 资产都留在本目录。
